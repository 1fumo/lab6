...
@app.route('/records/albums_by_band/<bandname>/', methods=['GET','DELETE'])
...
def create_a_record():
	if not request.json or not 'name' in request.json: 
		return jsonigy({'error:','the new record needs to have a band name'}), 400 
	new_record = {
		'name': request.json['name'],
		'album': request.json.get('albums','')
